...
public:
  ...
  void localSearch(vector<vector<double> > &mat) {
    ...
    // Primero hay que determinar los índices de los mejores
    for (unsigned i=0;i<CHROMOSOMES; i++){
      pair<double,int> p(v[i].fitness,i);
      q.push(p); // Los pares se ordenan por la primera componente (fitness), de mayor a menor
    }
    
    unsigned k = CHROMOSOMES*0.1; // Número de soluciones a las que aplicaré LS
    for (unsigned j=0;j<k; j++){
      v[q.top().second].localSearch(mat); // Se aplica LS a la solución en esa posición
      q.pop();
    }
  }
};
