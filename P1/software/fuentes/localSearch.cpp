...
unordered_set<int> randomSol(unsigned m, unsigned n){
...
void localSearch(vector<vector<double> > &mat, unsigned m) {
  ...
  unordered_set<int> solution = randomSol(m,mat.size());
  double variation=evaluateSolution(solution, mat);
  ...
  while (carryon){
    carryon=false;
    lowest=lowestContributor(solution, mat, min_contrib);
    
    // TODO: generar soluciones y probarlas
    if (contrib > min_contrib){
      variation = variation + contrib - min_contrib;
      carryon=true;
    }
  }
  cout << variation << endl;
